...
#[test]
fn owner_homes_require_fresh_no_loan_proof_and_bound_releases() -> ResourceResult<()> {
    ...
    assert_eq!(homes.home(data)?, second);
    let release = RemoteRelease {
        owner: data,
        from_task: id(0),
    };
    homes.remote_release(second, release)?;
    assert_eq!(
        homes
            .remote_release(second, release)
            .map_err(|error| error.code),
        Err("release-capacity")
    );
    assert_eq!(homes.drain_releases(second, 1), vec![release]);
    assert_eq!(
        homes.metrics(),
        OwnerMetrics {
            transfers: 1,
