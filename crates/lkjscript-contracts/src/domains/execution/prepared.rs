...
use super::super::{
    name, BYTECODE, NATIVE_LAYOUT, PACKAGE_LOCK, PREPARED_PROGRAM, RUNTIME_CALLS, VERIFIED_SSA,
};
...
pub(crate) fn prepared_program(dependencies: PreparedDependencies) -> ContractDescriptor {
    ContractDescriptor {
        ...
        items: vec![ContractItem::new("descriptor", ContractItemKind::Type)
            .semantic_order()
            ...
            .fact(fact(
                "contracts",
                "exact local compiler and execution contract digests",
            ))],
    }
}
