    ...
    fn verified_semantic_type_inner(
        ...
        Ok(match ty {
            ...
            Type::Enum { id, arguments, .. } => S::Enum {
                ...
                arguments: arguments
                    .iter()
                    .map(|item| self.verified_semantic_type(item))
                    .collect::<Result<_>>()?,
            },
            Type::Param(name) => S::Parameter(name.clone()),
            Type::List(item) => S::List(Box::new(self.verified_semantic_type(item)?)),
            Type::Fn { params, ret } => S::Function {
                parameters: params
                    .iter()
                    .map(|item| self.verified_semantic_type(item))
                    .collect::<Result<_>>()?,
                result: Box::new(self.verified_semantic_type(ret)?),
            },
            Type::Forall { vars, body } => S::ForAll {
                parameters: vars.clone(),
                body: Box::new(self.verified_semantic_type(body)?),
            },
        })
    }
}
