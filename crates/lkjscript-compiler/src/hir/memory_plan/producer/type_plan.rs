...
impl<'a> TypePlanner<'a> {
    fn new(program: &'a hir::Program) -> Result<Self> {
        Ok(Self {
            ...
            memo: HashMap::new(), facts: Vec::new(), drop_paths: Vec::new(),
    ...
    fn intern(&mut self, ty: &Type) -> Result<MemoryTypeFactId> {
        ...
        let root_projection = if derived.contains_dynamic_owner
            || matches!(ty, Type::Str | Type::Path)
            || (is_aggregate(ty) && derived.mode != MemoryAggregateMode::Copy) {
        ...
        self.facts.push(MemoryTypeFact {
            id, ty: memory_type(ty), mode: derived.mode, closure: derived.closure,
...
fn legacy(ty: &Type, reason: MemoryBlockerReason) -> DerivedType {
    DerivedType { mode: MemoryAggregateMode::ImmutableValue,
        closure: MemoryClosureFact { class: MemoryClosureClass::LegacyClosed,
