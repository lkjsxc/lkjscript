...
impl TypePlanner<'_> {
    ...
    fn producer_dependency(&self, root: &Type, child: &Type, role: lkjscript_contracts::ExecutableMemoryWitnessRole,
        ...
        let local = declaration_key(root).and_then(|left| self.graph.component(&left).map(|component|(left,component)))
            .and_then(|(left,component)| declaration_key(child).filter(|right| self.graph.is_recursive(&left)
        ...
        let target = if let Some(identity)=local { T::LocalSemantic(identity) } else {
            let fact = self.memo.get(child).copied().and_then(|id| self.facts.get(id.index()?))
                .ok_or_else(|| Error::msg("witness dependency child was not independently interned"))?;
