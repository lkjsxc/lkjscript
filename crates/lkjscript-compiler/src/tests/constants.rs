...
fn bytecode_constants_preserve_numeric_source_types() {
    let integer =
        "main/\nsig/\ninputs/\n/inputs\noutput/\ni64\n/output\n/sig\n9223372036854775807\n/main\n";
    let integer = compile_source(integer, "integer.lkjscript").expect("compile I64 source");
    assert!(integer
        .bytecode()
        .constants()
        .iter()
    ...
    let float = compile_source(float, "float.lkjscript").expect("compile F64 source");
    assert!(float
        .bytecode()
        .constants()
        .iter()
