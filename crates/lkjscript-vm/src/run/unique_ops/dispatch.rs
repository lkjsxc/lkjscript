pub(super) fn dispatch(vm: &mut Vm<'_>, op: u8) -> Result<()> {
    ...
    match op {
        ...
        Op::ByteSliceLen => {
            let view = vm.pop()?;
            let len = if super::structural_ops::is_byte_view(vm, view) {
                super::structural_ops::len(vm, view)?
            } else {
                vm.unique.len(view)?
            };
        ...
        Op::ByteSliceRef => {
            ...
            let byte = if super::structural_ops::is_byte_view(vm, view) {
                super::structural_ops::byte_at(vm, view, index)?
            } else {
                vm.unique.byte_at(view, index)?
            };
