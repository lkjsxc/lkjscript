use lkjscript_core::{
    ...
    StructuralFieldPath, StructuralFieldRoute, StructuralKind, StructuralPayloadRef,
    StructuralProjection, VariantId,
};
    ...
    let view = invocation_mut(vm)?
        ...
        .borrow_projected(
            ...
            StructuralProjection::Field {
                path: StructuralFieldPath::new(vec![reference.field]),
                expected,
            },
            false,
        )
    ...
    match reference.result.route {
        StructuralFieldRoute::Copy => {
            let value = invocation(vm)?
                ...
                .projected(view)
                .map_err(map_value_error)?;
            let result = semantic_to_value(vm.chunk, value)?;
            invocation_mut(vm)?
                .runtime
                .end_view(view)
                .map_err(map_value_error)?;
        ...
        StructuralFieldRoute::Structural(type_id) => {
            let representation = view_representation_for_type(vm.chunk, type_id)?;
            let value = invocation_mut(vm)?.register_view(view, representation, expected, false)?;
            vm.push(value);
        }
...
fn string_utf8_view<J: RuntimeTier>(vm: &mut Vm<'_, J>) -> Result<()> {
    ...
    let view = invocation_mut(vm)?
        ...
        .map_err(map_value_error)?;
    let value = invocation_mut(vm)?.register_view(view, view_representation, expected, true)?;
    vm.push(value);
    Ok(())
}

include!("aggregate/support.rs");
