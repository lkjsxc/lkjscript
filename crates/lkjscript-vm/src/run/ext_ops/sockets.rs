...
pub(super) fn dispatch<J: RuntimeTier>(vm: &mut Vm<'_, J>, op: u8) -> Result<bool> {
    match op {
        ...
        x if x == Op::SysRecv as u8 => {
            ...
            match vm.resources.sys_recv(&mut vm.arena, handle) {
                Ok(value) => {
                    let result = crate::host_ext::result_ok(&mut vm.arena, value)?;
                    vm.push(result);
                }
                ...
                Err(crate::host_ext::SocketReceiveError::Utf8(error)) => {
                    let result = crate::host_ext::system_utf8_error(&mut vm.arena, error)?;
                    vm.push(result);
                }
            }
            Ok(true)
