...
impl RuntimeSystem {
    ...
    pub fn invoke(
        ...
        let execution = if let Some(chunk) = &admission.chunk {
            Ok(run_chunk(chunk, &admission.inputs, &admission.config))
        } else if let Some(process) = &admission.process {
            let result = process
                .lock()
                ...
                .and_then(|mut process| {
                    process
                        ...
                        .map_err(RuntimeError::ProcessCell)
                });
            result.and_then(|(outcome, output, flushes)| {
                relay_process_output(&admission, &output, flushes)?;
                Ok(outcome)
            })
        } else {
            Err(RuntimeError::ProcessCell(
                "running application has no execution cell".into(),
            ))
        };
