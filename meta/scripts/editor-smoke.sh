...
printf 'seed\n' > "$TMP"
printf 'iHELLO \x1b:wq\n' | "$LKJ" run "$ROOT/examples/texteditor/main.lkjml" "$TMP" >/dev/null
grep -q 'HELLO seed' "$TMP"
printf 'q' | "$LKJ" run "$ROOT/examples/texteditor/main.lkjml" "$TMP" >/dev/null
grep -q 'HELLO seed' "$TMP"
# Missing path -> new file on :wq.
printf 'ihi\x1b:wq\n' | "$LKJ" run "$ROOT/examples/texteditor/main.lkjml" "$NEW" >/dev/null
test -f "$NEW"
grep -q 'hi' "$NEW"
# CR+LF: two content lines then quit; ensure redraw has CR before LF.
printf 'a\nb\n' > "$TMP"
printf 'q' | "$LKJ" run "$ROOT/examples/texteditor/main.lkjml" "$TMP" >"$TMP_DIR/crlf.out"
grep -q $'\r' "$TMP_DIR/crlf.out"
# Command-line paint: :wq must appear in the redraw stream.
printf 'iZ\x1b:wq\n' | "$LKJ" run "$ROOT/examples/texteditor/main.lkjml" "$TMP" >"$TMP_DIR/cmd.out"
