...
Implemented work is intentionally absent. Graph 5 authority, CLI 11, offline minimal, command, and
HTTP recipes, exact built-in standard dependency, normalized check/build/pure-run, artifact 10,
maintained standard/`lkjournal` artifacts, reviewed authored change, normalized query, standalone
artifact-10 service/worker deployment, copied-binary HTTP authoring acceptance, and direct
predecessor runtime deletion are current architecture. The immutable `v0.1.6`
GitHub Release is the current public `x86_64-unknown-linux-gnu` distribution path; its measured
runtime floor and exact evidence are current facts rather than multi-platform admission. It does
not contain the current-main HTTP recipe.
...
1. **Publish the verified next patch release.** From exact clean current content, rerun release
   preparation, create one new annotated patch tag, publish immutable assets, and prove the complete
   HTTP authoring/change/check/build/serve/restart workflow from anonymous exact-tag and
   `releases/latest` downloads. Do not alter v0.1.6 or infer public availability from a local
   candidate.
2. **Dependency-closed generic HTTP topology authoring.** Expose component, requirement, port, and
   target change records only when a second maintained workflow needs topology beyond the closed
...
6. **Measured static/musl Linux portability.** Admit a musl asset only after its exact candidate
...
7. **Additional platform releases one target at a time.** Treat each architecture and operating
   ...
   build matrix.
8. **Normalized package management and removed operational workflows.** Introduce package
...
9. **Large-graph compiler, artifact, and retention evidence.** Extend current locality and command
