...
Implemented work is intentionally absent. Meaning graph contract 4, direct CLI v9, binary-only
minimal project creation, normalized change v4 with reviewed ownership-closure deletion and
logical plan v1, normalized owner/name/relation query v3, batched persistent Merkle maps, exact-ID
imports and targets,
...
3. **Remaining authored semantic operations.** Add dependency-closed operations such as move,
...
4. **Later package, service, and worker cutovers.** After normalized package/artifact and maintained
...
5. **Extend incremental validation and lowering.** Generalize beyond pure-body replacement,
...
6. **Large-graph physical evidence and safe retention.** Extend the retained 10,000-owner query
   ...
   measurements.
7. **Persistent semantic conflict workflow.** Convert conflicted merges into typed non-executable
   drafts and add bounded conflict list/detail/resolve operations. Accepted HEAD remains
   conflict-free.
8. **Compiler specialization and language abstraction from consumers.** Retain clean/incremental
...
9. **Adapter and agent economy.** Add PostgreSQL cancellation, live S3 conformance, streaming,
