...
**Mixed status.** Dedicated Unit, exact three-arm `if`, typed empty lists,
explicit Option/no-nil semantics, explicit equality families, immutable nominal
products, local-only mutation, product-state migration, explicit main,
fixed-point function effects, verified typed SSA, the callable host-independent
Linux x86-64 baseline tier, and the forced proof-checked optimizing slice are
**Current**. Broader ownership, host-native transitions, automatic proof
promotion, OSR, and later semantic additions remain accepted or deferred only
where their authorities say so. [Current State](../../current-state.md) records
the exact boundary.
