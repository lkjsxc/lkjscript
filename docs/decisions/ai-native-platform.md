...
**Accepted Target.** The existing Edition 1 parser, typed HIR, verified SSA,
...
the accepted destination and dependency order; it does not make Semantic
Source, Edition 2, packages, general ownership, AOT, Wasm, concurrency, or
self-hosting Current.

The first selected implementation slice is the versioned Semantic Source and
agent-protocol foundation in [Semantic Source And Agent
Protocol](semantic-source-and-agent-protocol.md). The previously selected
automatic optimizing-promotion slice remains a valid measured experiment but is
no longer the next repository-wide implementation priority.
...
| Runtime JIT is the only final deployment strategy; offline PGO/cache are permanently rejected | The measured execution portfolio in [Execution Portfolio](execution-portfolio.md) | Current JIT modes and evidence remain unchanged |
