...
## Blocked Persistent Collections

PaC-trees, SPAA 2022, [DOI 10.1145/3519939.3523733](https://doi.org/10.1145/3519939.3523733)
and [arXiv 2204.06077](https://arxiv.org/abs/2204.06077), demonstrate that
array-like blocks can reduce persistent-collection per-element overhead while
retaining functional snapshots. Lkjscript adopts only bounded grouping and
same-commit size measurement for list segments. Balanced-tree algorithms,
delta compression, and source-visible block size are rejected for linked-list
semantics. Prepend/rest copy amplification, retained tails, release p99, and
nontrivial element storage remain implementation-specific uncertainties.

## Adopted Synthesis And Falsification
...
verified destination passing and reuse, flat recursive images, static memory
witnesses, and explicit SSA ownership events.

The list experiment predeclares unique structural copying, ordinary regions,
one-node sealed regions, sealed segments with bounded capacities, and a private
precise immutable per-node-count control. Workloads include short and long
chains, repeated rest, shared and branching tails, nested values, compiler and
editor snapshots, long-lived app state, Web configuration, and immutable game
assets. It records allocations/bytes/RSS, over-retention, copies, dependencies,
retain/release and atomic operations, root traffic, release work and operation
p50/p95/p99, compile time, native code size, and AI repair burden.

Per-node counting is permitted only when retained same-commit evidence shows a
material failure of borrowing, unique reuse, copying, ordinary regions, sealed
regions, and segmentation. Otherwise the production candidate is deleted.
Concurrent counting and shared mutable capsules remain deferred. No cited
result justifies tracing fallback, unbounded release, source lifetime syntax, or
moving proof obligations to users.
