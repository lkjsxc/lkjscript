...
creation covers non-generic products, generic or non-generic functions and enums, including direct
self-recursive enums, and one `main` with zero or more ordered explicit capability parameters;
function and entry bodies begin as typed holes. Each main parameter is a stable entity owned by main
and visible in its body hole. Main creation applies the same exact capability-only, sorted-kind,
unique-name, and result restrictions as source import before publication. The generalized enum and
function creation operations accept zero or more ordered type-parameter drafts. A declaration-local
opaque binder handle may occur at arbitrary depth in the creation-only `DeclarationType`; it has only
that edit's lifetime and never enters a snapshot, query, projection, or diff. Enum fields use this
declaration type directly. `DeclarationType::CurrentEnum { arguments }` is valid only within the
fields of its owning `CreateEnum`, requires the exact explicit argument count, and may contain the
same declaration-local binder handles. Enum staging allocates the stable enum and binder entities
before field resolution, then resolves the marker once to the ordinary stable `SemanticType::Enum`
constructor and canonical private enum type. The marker has no source spelling or identity and never
enters the semantic program, indexes, queries, projections, continuations, or diff. Using it in
function or main input rejects before publication. Resolving the marker does not bypass canonical
compiler acceptance: complete imported and source-free uses of changing recursive instantiations
receive the same memory-planning result. Enum parameters are unbounded, may be phantom, and do not
carry trait bounds. Function staging additionally validates exact trait identities and
bounds. Staging allocates each declaration, binder, and member or value-parameter entity before
constructing the same canonical enum or universal function facts used by the source importer. Public
published type inputs and query results use one exact recursive `SemanticType`, whose nominal and binder cases carry stable workspace identities rather than product
