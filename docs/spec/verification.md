...
| Native development-tool source, exact derived artifact, fresh authoring, canonical re-entry, independent output and detached old/new execution | Existing `workspace_tests` through `tests/public_cli/native_guides.rs`; `generated_public_guides` separately checks published document bytes |
