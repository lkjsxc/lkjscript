...
| Native development tools | `tools/native-guides/project`, `tools/native-policy/project`, `platform/native_tool.rs` | ordinary guide rendering and repository-policy decisions, derived pure bundles, shared strict grant-free embedding; host-owned typed observations | registry/Git/filesystem authority, semantic publication, capability grants, or compiler self-hosting |
