...
| Native development tools | `tools/native-guides/project`, `platform/contract/native_guides.rs` | native operation/diagnostic document layout and escaping, derived embedded pure artifact; host-owned typed registry input | registry authority, semantic publication, capability grants, or compiler self-hosting |
