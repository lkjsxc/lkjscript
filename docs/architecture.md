...
| Native development tools | `tools/native-guides/project`, `platform/contract/native_guides.rs` | native layout, escaping and exact-reference selection for all eight guides, derived embedded pure artifact; host-owned typed registry input | registry authority, semantic publication, capability grants, or compiler self-hosting |
