...
Use focused feedback while developing and dependency-complete acceptance after relevant inputs stabilize. Honor required gates and protections. Do not repeat overlapping broad suites for reassurance. Concurrent Cargo configurations can replace shared outputs through feature unification: honor producer dependencies and preserve immutable executable copies. Select test executables from Cargo's reported outputs, not guessed hashed filenames.
