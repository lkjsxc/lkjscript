...
Historical requests are context, not permanent architecture requirements. Re-evaluate them against the current specification, checkout, product path, measurements, and long-term quality. Do not preserve a mechanism merely because it was previously discussed or implemented.

Do not ask the user to choose among technical alternatives when repository evidence, executable tests, a focused experiment, profiling, or a reversible assumption can decide. Ask only when a genuinely external requirement is missing and no safe assumption can unblock the task.

Do not destroy unrelated local work, external data, credentials, or host state. Authorization to redesign this repository is not authorization to erase unrelated state.

The only permanently fixed property of the program file format is the `.lkjscript` extension. The current bytes, grammar, textuality, encoding, source layout, CLI, package format, bytecode, cache format, and internal Rust APIs are provisional unless an accepted specification says otherwise.

## Mission

Build `lkjscript` into an AI-primary, statically typed, memory-safe, high-performance programming language and implementation.

AI-primary means that an agent can discover, construct, inspect, transform, validate, compile, execute, and review programs through deterministic, precise, compact, composable interfaces. It does not mean that model inference belongs in a compiler, runtime, validator, optimizer, storage layer, or correctness boundary.

The product direction is:

- one syntax-independent semantic program authority;
- first-class program construction without source-text bootstrapping;
- stable logical identities rather than fragile textual coordinates;
- explicit incomplete states rather than fabricated executable placeholders;
- typed, atomic, revision-checked edits;
- direct access to type, scope, effect, capability, ownership, dependency, and diagnostic facts;
...
- a complete generic execution route, with optional specialization that may decline before effects;
- low agent round-trip count and failures that describe legal next actions;
- human-reviewable projections and reproducible builds; and
- strong end-to-end performance, including startup, compilation, edit/query latency, runtime throughput, memory use, allocation, copying, serialization, generated code, and binary size.

Correctness must remain deterministic and locally decidable. A model may propose an operation; deterministic implementation machinery decides whether it is valid.

Do not optimize semantics for one model's tokenizer. Do not make the system opaque merely to call it AI-native. Humans and ordinary tools must retain precise inspection, diagnostics, validation, semantic diffing, and review.
...
5. `docs/status.md` reports current implementation;
6. `docs/architecture.md` explains current responsibilities and boundaries;
7. `docs/performance.md` owns measurement method and compact evidence;
...
Use labels such as **Current**, **Target**, **Hypothesis**, **Historical**, **Unknown**, and **Blocked** when ambiguity would otherwise result.

Do not create another authority layer from task prompts, global revisions, prose digests, registries, evidence ledgers, generated inventories, closure graphs, checkpoints, handoffs, or completion capsules.
...
Maintain one active language definition, semantic program authority, compiler path, generic production execution route, ownership model, package model, and documentation authority model.

When replacing a mechanism, prefer a dependency-closed cutover. Delete displaced implementations, adapters, aliases, feature flags, compatibility tests, stale documentation, and dead dependencies in the same coherent change.

Do not keep permanent `v2`, `next`, `legacy`, `archive`, or `compat` implementations. Do not keep old and new parsers, semantic authorities, compilers, runtimes, or storage formats in parity. Git history is the migration record for discarded experimental work.

A small independent evaluator or model may remain when it provides a useful test oracle. It is not automatically a second production engine.

Names and current crate boundaries have no authority. Preserve, merge, split, rename, or delete components according to cohesion, safety, true platform ownership, useful compile isolation, measured cost, and current consumers.

## Representation discipline

The authoritative semantic state must be able to exist without source text, formatting, paths, spans, parser nodes, or source hashes. Those are optional import, presentation, provenance, or trust-boundary attachments.

Text import may create semantic state. Compilation, semantic editing, and queries must not require rendering and reparsing text.

Do not satisfy an internal invariant with fake semantic data. Do not invent dummy source files, placeholder paths, fabricated hashes, hidden valid bodies beneath holes, synthetic entry points, or fake declarations because an internal type currently requires them. Correct the representation or boundary.

Use an honest source-optional origin model for semantic facts. A source-backed fact may name source provenance; a source-free fact must remain source-free. Never smuggle source-free meaning through a reserved or dummy `SourceId`.
...
Incomplete semantic state is valid editing state. Missing declarations, bodies, or expressions must be explicit blockers or holes. Do not retain executable fallback meaning behind an incomplete node.

The `.lkjscript` encoding is deliberately open. Do not redesign it without a present storage, transfer, integrity, startup, or tooling requirement. An opaque or binary primary representation must still have deterministic first-party inspection, validation, querying, editing, diffing, export/projection, malformed-data handling, and failure-atomic publication.

Do not infer that source-free authoring requires a database, journal, protocol, daemon, or distributed store. Complete and measure the in-process semantic workflow first.
...
Do not build speculative daemons, persistence layers, journals, distributed storage, CRDTs, schedulers, resource topologies, process-cell systems, universal registries, plugin frameworks, general incremental frameworks, proof ecosystems, wire protocols, target matrices, multi-tier JIT policy, deoptimization machinery, custom allocators, custom schedulers, or storage engines. A current task may introduce such a mechanism only with a present consumer, demonstrated boundary, measured need, acceptance criteria, and reversal condition.

Do not solve complexity by adding bookkeeping around it. First remove unused work, repeated scans, reconstruction, duplicated facts, and unnecessary boundaries.
...
Use checked arithmetic and checked narrowing for sizes, offsets, identities, code locations, handles, and indexes. User-controlled depth must not consume unbounded native stack. Use iterative traversal or a justified heap-backed work stack. Never silently truncate a complete result; paginate, stream, return an explicit partial result, or fail.
...
Follow the accepted ownership specification. Ordinary execution is collector-free and non-tracing. Do not introduce tracing collection, hidden reference-counting semantics, raw-pointer language surface, retain/release, or general `free` merely to simplify an implementation. A memory-semantic change requires a specification change and one complete cutover, not parallel GC and non-GC modes.

Preserve exact move and borrow laws, deterministic cleanup where promised, cleanup on normal, error, cancellation, and early-exit paths, no double release, stack-safe destruction, explicit host-resource ownership, and failure-atomic publication.
...
Relevant evidence may include wall time, phase time, startup, throughput, edit/query latency, peak and retained memory, allocations, bytes copied, bytes parsed/rendered/serialized/hashed/generated, repeated traversal counts, scale behavior, generated-code size, and binary size.
...
The task owns the objective, not an unverified proposed mechanism. Change course when executable evidence invalidates the suggested implementation.
...
Complete one coherent vertical, update the roadmap, identify the next problem, and stop. Do not begin the next roadmap item merely because time remains.
...
Remove numbered shards, include-only facades, one-child directory ladders, artificial tiny modules, and redundant model or conversion layers when recombination improves comprehension. Do not reorganize unrelated code for aesthetics.
...
- stable identity, namespaces, generations, revisions, and deterministic ordering;
...
Write a decision record only when a choice is durable, non-obvious, expensive to rediscover, and has a meaningful reversal condition. Do not describe a target architecture as current.
...
The final report must separate implemented work, semantic or architectural changes, important deletions, measurements, tests and exact commands, commit and push state, untested paths, remaining risks, deliberately deferred work, and the next highest-leverage problem.
