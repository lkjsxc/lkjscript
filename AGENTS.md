...
Reuse evidence only with valid source, verifier, environment, workload, input and trust bindings. A cache is not a correctness certificate; relevant changes or failures invalidate affected proof. Retain authenticated originals and test reconstructed copies, preserving required bytes, metadata and context. Never fabricate missing originals, rewrite failed receipts into passes or claim reconstructed historical context as a fresh live environment observation.
...
Use only the release-scoped operations authorized by current instructions and `docs/release.md`. Do not change immutability, credentials or access controls. Public closure requires the prescribed original-reader admission, anonymous acquisition and distributed/installed behavior; a green subjob, main push or uploaded asset is insufficient. Retain originals before expiry for their actual resumption need. Do not restart a healthy/completed valid pipeline or change frozen inputs merely to append guidance or a report.
