//! Typed operation structures used by canonical module shards and the independent text oracle.
...
#[derive(Decode, Encode, Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
#[serde(tag = "kind", content = "data", rename_all = "snake_case")]
pub enum Type {
    Unit,
    Bool,
...
mod source_oracle {
    use super::*;
    use crate::platform::syntax::{Form, FormKind, SourceDocument};

    pub(crate) fn parse_module(document: &SourceDocument) -> Result<Module, Diagnostic> {
    ...
    fn parse_match(
        ...
        for item in &items[2..] {
            ...
            if !(3..=4).contains(&arm.len())
                || atom(document, &arm[0], "match_arm_keyword")? != "case"
            {
                return Err(at(
                    document,
                    item,
    ...
    fn validate_identifier(value: &str, kind: IdentifierKind) -> Result<(), String> {
        ...
        if !matches!(kind, IdentifierKind::Module | IdentifierKind::Qualified)
            && segments.len() != 1
        {
...
#[cfg(test)]
pub(crate) use source_oracle::parse_module;
