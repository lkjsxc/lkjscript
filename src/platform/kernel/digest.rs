//! Typed, domain-separated Graph 10 object identities.
            ...
            fn decode<D: Decoder<Context = Context>>(decoder: &mut D) -> Result<Self, DecodeError> {
                ...
                if tag != $tag {
                    return Err(DecodeError::OtherString(format!(
                        "foreign Graph 10 digest domain tag {tag}; expected {}",
                        $tag
                    )));
                }
...
fn decode_digest(value: &str) -> Result<[u8; DIGEST_BYTES], Diagnostic> {
    if value.len() != DIGEST_BYTES * 2 {
        return Err(digest_error(
            ...
            "Graph 10 digest must contain 64 lowercase hexadecimal characters",
        ));
    }
...
fn invalid_hex(value: &str) -> Diagnostic {
    digest_error(
        "kernel_digest_hex",
        format!("Graph 10 digest '{value}' is not canonical lowercase hexadecimal"),
    )
}
