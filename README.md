...
create products, generic or non-generic functions and enums (including direct self-recursive enums),
and an entry point with ordered explicit capability parameters, then fill real typed holes with flat
non-recursive drafts. Enum and function creation use declaration-local binder handles only until
atomic publication assigns stable declaration-owned type-parameter entities. Inside `CreateEnum`,
`DeclarationType::CurrentEnum` names only the enum being created and resolves to its staged stable
identity; the marker never enters a snapshot. Main capability parameters and ordinary value
